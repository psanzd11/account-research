...
def load_trace() -> pd.DataFrame:
    ...
    def _cost(row):
        ...
        if not p:
            return 0.0
        return (row["input_tokens"] * p["in"] + row["output_tokens"] * p["out"]) / 1_000_000
...
c2.metric("Input tokens", _fmt_tokens(total_in))
c3.metric("Output tokens", _fmt_tokens(total_out))
c4.metric(
    "Spend",
    f"${total_usd:,.2f}",
    delta=f"≈ ${total_usd / max(total_calls, 1):.2f} / call",
)
...
with st.expander(f"Show all {total_calls:,} calls", expanded=False):
    ...
    display_cols = [
        "ts", "agent_label", "model", "input_tokens", "output_tokens",
        "usd", "elapsed_ms", "stop_reason", "error",
    ]
